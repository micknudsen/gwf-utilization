...
def utilization(obj, targets):
    ...
    rows = [['Target', 'Cores', 'Allocated Walltime', 'Used Walltime', 'Allocated Memory', 'Memory Used', 'Allocated CPU Time', 'Used CPU Time', 'Memory Utilization', 'CPU Utilization']]
    for target, job in zip(matches, get_jobs(job_ids)):
        rows.append([target.name,
                     job.allocated_cores,
                     job.allocated_walltime,
                     job.used_walltime,
